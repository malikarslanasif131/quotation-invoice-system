...
<form method="POST" action="{{ route('quotations.update', $quotation) }}">
    ...
    <h5>Items</h5>
    <div id="items-list">
        @foreach(old('items', $quotation->items->toArray()) as $i => $qitem)
            <div class="row item-row mb-2">
                    ...
                    <select name="items[{{ $i }}][item_id]" class="form-control" required>
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}"
                                @if((isset($qitem['item_id']) ? $qitem['item_id'] : $qitem['item']['id']) == $item->id) selected @endif>
                ...
                <div class="col-md-3">
                    <input type="number" name="items[{{ $i }}][qty]" class="form-control"
                        value="{{ isset($qitem['qty']) ? $qitem['qty'] : $qitem['qty'] }}" min="1" required>
                </div>
                <div class="col-md-2">
